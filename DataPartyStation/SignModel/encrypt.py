...
def save_signed_model_file(signed_models, local_save, receiver_address, party_name, m):
    ...
    if receiver_address != False:
        ...
        wb_model_file = open("/output/%s_%s.enc" %(party_name, m), "w")
        ...
        try: 
            res = requests.post(url=receiver_address+'/addFile',
                files={"fileObj": open("/output/%s_%s.enc" %(party_name, m), 'r')})
...
def main():
    ...
    for m in model_files:
        ...
        try:
            ...
            save_signed_model_file(signed_models, local_save, receiver_address, party_name, m)
